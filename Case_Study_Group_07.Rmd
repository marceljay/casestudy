...
Our main task as project manger is to select a vehicle type, which is the most critical in terms of the production time. The production time refers to the maximum time between the production of a part and the production  of  the  entire  vehicle. The visualization of our data shows big differences between the existing vehicle types. While the types 11 and 12 have a relativly low production time, the types 21 and 22 are far great with many extreme outliers. This fact ist best shown in the single vehicle type per boxplot view below.
...
If you hover with the cursor over the boxplot, it displays the median, the upper and lower quartiles. Furthermore it shows both the upper and lower fence as well as the minimum and maximum production time. This is exemplary shown for vehicle type 11 in the image below.
...
Vehicle type 22 outperfoms vehicle type 21 in almost all of the visualized data. The only exception is the minimum production time, which is 10 days for both vehicle types. Suprisingly this is a lower minimum value than that of vehicle type 11 and 12, which is 11 days. However, the maximum production time for both vehicle type 21 and vehicle type 22 is significantly greater than the ones of vehicle type 11 and vehicle type 12. Vehicle type 21 has a slightly higher value with 671 days than vehicle type 22 with 666 days. The upper and lower fence of vehicle type 21 are both 2 days greater than the ones of vehicle type 21. They are 60 days and 44 days for vehicle type 21 and 58 days and 42 days for vehicle type 22. 
...
Thus from the analysis of the boxplots in our shiny app derives the clear conclusion, that vehicle type 21 is the most critical one. It is our job as project manager to take countermeasures by improving certain processes. If we would like to take action, it is necessary to to know in which factories vehicle type 21 is produced. This task can easily be done with our shiny app. By selecting the radio button divide boxplot by OEM factory the factory locations are displayed.

At first we can have look where the most extreme outliers are produced in general by selecting a production time range between 100 days and 671 days. As depicted in the following image the problematic factories are Goettingen and Regensburg.
...
Our main focus lies on vehicle type 21, therefore we select the corresponding checkbox. As shown in the image below, all vehicles of type 21 are produced in Goettingen. This is the OEM factory, which needs to imrpove.
