...
This document provides all major steps and important information regarding the execution of the Case Study for Group 7 of the IDA Course. The output will be a complete data frame with essential data needed for the Shiny App. Since this objective involves importing a large amount of data, the import process will approximately take over 20 minutes, as will the rendering of this Rmarkdown file. The resulting Shiny application will also have difficulties processing and visualizing the final data frame, resulting in delayed visualizations of diagrams and tables.
...
# Import & Tidy of all segments (Wrangle)
...
All the data frames from the list are then joined into one single data frame for all the parts. Since this makes the original `df_list` redundant, it can now be removed to save memory as the import process tends to reserve a large amount. 
```{r link_part_df_list}
...
link_part_df_list <- function() {
  ...
  # remove redundant list to save memory
  df_list <<- NULL
}
...
We merged all of the component data frames to minimize the amount of action, necessary to adjust the data. Furthermore we minimized the amount of data for further analysis by deleting the unnecessary column X1, which contained only index information. Again, the redundant single vehicle data frames are removed to save memory.
...
link_BE_vehicles <- function() {
  ...
  # remove redundant data frames to save memory
  df_B11 <<- NULL
  df_B12 <<- NULL
  df_B21 <<- NULL
  df_B22 <<- NULL
...
* Lastly, the remaining part data frame containing its production date is merged and the gap between vehicle, components and parts is bridged. The previously used merged data frames can be removed to save memory.
...
vehicleCompPart_merger <- function() {
  ...
  part_df <<- NULL
  df <<- NULL
  merged_compBE_df <<- NULL
  merged_vehicleBE_df <<- NULL
  merged_vehicleToComp_df <<- NULL
}
```
...
For our shiny app we needed a new column in our master data frame, which contains the maximum time between production of a part and production of the entire vehicle. Therefore we created a column which listed this time difference for every part. In the next step we created a list containg just the maximum time difference for every single vehicle. Again, `master_df` is removed to save memory.
...
prepare_masterdf_1 <- function() {
  ...
  # Remove master_df to save memory
  master_df <<- NULL
}
...
Due to the large memory allocation of the imported data, the Shiny App might have delays when changing input.
...
Our main task as project manager is to select a vehicle type, which is the most critical in terms of the production time. The production time refers to the maximum time between the production of a part and the production  of  the  entire  vehicle. The visualization of our data shows big differences between the existing vehicle types. While the types 11 and 12 have a relatively low production time, for the types 21 and 22 much longer production times were revealed with many extreme outliers and higher occurrences. This fact is best shown in the single vehicle type per boxplot view below.


```{r, out.width = "400px"}

knitr::include_graphics("Additional_Files_Group_07/shiny_app_pics/all_vehicle_types.jpg")

```


If you hover with the cursor over the boxplot, it displays the median, the upper and lower quartiles. Furthermore it shows both the upper and lower fence as well as the minimum and maximum production time. As an example, this is shown for vehicle type 11 in the image below.


```{r, out.width = "500px"}

knitr::include_graphics("Additional_Files_Group_07/shiny_app_pics/vehicle_type_11.jpg")

```


By looking at the visualized data, it becomes clear that statistically, vehicle type 11 tends to have the lowest production times, even lower than vehicle type 12. The median of vehicle type 11 is 27 days, the upper quartile is 30 days, while the lower quartile is 26 days. The minimum production time is 11 days and the maximum production time is 48 days. In comparison to that, vehicle type 12 has a slightly higher median with 28 days, an equaly low upper quartile with 30 days and a lower quartile of 27 days. The minimum production time is also 11 days. While the maximum production time is a little higher with 49 days.

The production times of vehicle type 11 and 12 outpeform the ones of 21 and 22 as depicted in the first graphic. It is our task as the project manger to focus on the critical ones, so we can initiate processes for improvements. The boxplots of vehicle type 21 and 22 look very similar in our shiny app. A closer look at the data is needed to decide which of them is more critical. 


```{r, out.width = "400px"}

knitr::include_graphics("Additional_Files_Group_07/shiny_app_pics/vehicle_types_21_22.jpg")

```

Vehicle type 22 outperfoms vehicle type 21 in almost all of the visualized data. The only exception is the minimum production time, which is 10 days for both vehicle types. Suprisingly this is a lower minimum value than that of vehicle type 11 and 12, which is 11 days. However, the maximum production time for both vehicle type 21 and vehicle type 22 is significantly greater than the ones of vehicle type 11 and vehicle type 12, and occurs much more frequently. Vehicle type 21 has a slightly higher value with 671 days than vehicle type 22 with 666 days. The upper and lower fence of vehicle type 21 are both 2 days greater than the ones of vehicle type 21. They are 60 days and 44 days for vehicle type 21 and 58 days and 42 days for vehicle type 22. 

In terms of outliers both vehicle types achieve comparable results. This can easily be shown in our shiny app by adjusting the slider input for the production time range. The table in the third tab updates and shows that both vehicle types have an almost equally high amount of extreme outliers. 

The most important values displayed are the median and the upper and lower quartile, because they depict the vast majority of our data. In this regard vehicle type 22 has lower and thus less critical values for production time. The upper quartile of vehicle type 22 is 52 days and thus 2 days lower than vehicle type 21 upper quartile, which is 54 days. The median of vehicle type 22 is also smaller. It is 49 days compared to 53 days for vehicle type 21. Finally the lower quartile is 48 days for vehicle type 22 and 50 days for vehicle type 21. This is another difference of two days in favour of vehicle type 22.


Thus from the analysis of the box plots in our shiny app derives the clear conclusion, that vehicle type 21 is the most critical one. It is our job as project manager to take countermeasures by improving certain processes. If we would like to take action, it is necessary to to know in which factories vehicles of type 21 are produced. This task can easily be done with our shiny app. By selecting the radio button `divide boxplot by OEM factory`, the factory locations are displayed.

At first we can have a look at where the most extreme outliers are produced in general by selecting a production time range between 100 days and 671 days. As depicted in the following image the problematic factories are Goettingen and Regensburg.


```{r, out.width = "400px"}

knitr::include_graphics("Additional_Files_Group_07/shiny_app_pics/vehicle_type_outlier_factory.jpg")

```

Our main focus lies on vehicle type 21, therefore we select the corresponding checkbox. As shown in the image below, all vehicles of type 21 are produced in Goettingen. This is the OEM factory which needs to improve.


```{r, out.width = "400px"}

knitr::include_graphics("Additional_Files_Group_07/shiny_app_pics/vehicle_type_21_factory.jpg")
