...
While skimming through the files in the directories `Einzelteil`, `Komponente` and `Fahrzeug` using external text editors, MS excel and in command line with head() and tail(), it became clear that the often sequentially named .csv and .txt files within contain columns with date-like values such as `origin`, while files in the component and vehicle directories containing `Bestandteile` in their names serve as keys to match columns in parts, components and vehicles together.
...
Lastly, the function which executes all vehicle data wrangle related functions is defined:

```{r vehicle startImport}
# execute all the previously explained functions for importing vehicle data
startImportVehicle <- function() {
