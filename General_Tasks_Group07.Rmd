...
```{r}
Logistics_Delay <- data.frame(delay = logistic_delay_K7$Wareneingang - component_K7$Produktionsdatum)
```

Distribution of Logistics Delay
```{r}
hist(as.numeric(Logistics_Delay$delay))
```
Normal distribution????
...
...
It makes sense to sotore available data in seperate files instead of saving everything in a huge table for several reasons. Seperate data sets connected by a primary key will only contain specific information ensuring data dependencies make sense. Thus extraction from specific data will be easier. Furthermore duplicate data throughout can be easier identified and eliminated, enhancing the efficiency of the code and saving memory space/cache.Furthermore the different tables will be more flexible for adaptations and newly introduced data. 
...
```{r}
Vehicles_Dortmund_K7 <- left_join(components_vehicles, registrations, by = c("ID_Fahrzeug" = "IDNummer")) %>%
...
left_join(registrations, by = c("ID_Fahrzeug" = "IDNummer")) 
Vehicle_Accident
